...
fn record_bench(engine: &Kosa, slots: &[u64], required: usize) -> BenchResult {
    ...
    for &slot in slots {
        ...
        let result = engine.read(slot, required).expect("read failed");
        assert!(result.is_some(), "Data should exist and pass CRC validation");
...
fn single_tx_read_latency() -> BenchResult {
    ...
    let engine = Kosa::new(cfg).expect("new Kosa");
    ...
    println!("-> Running single-thread read benchmark...");
    record_bench(&engine, &slots, required)
}

fn multi_tx_read_latency() -> BenchResult {
    let (_dir, cfg) = prep_init();
    let engine = sync::Arc::new(Kosa::new(cfg).expect("new Kosa"));
    ...
    println!("-> Running multi-thread read benchmark...");
